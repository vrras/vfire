...
class Vfire {
  constructor(credential) {
    ...
    const firestore = init.firebase.default.firestore();
    const storage = init.firebase.default.storage();
    const auth = init.firebase.default.auth();
    const timestamp = init.firebase.default.firestore.FieldValue.serverTimestamp();
        ...
        uploadTask.on(
          ...
          (snapshot) => {
            switch (snapshot.state) {
              case init.firebase.default.storage.TaskState.PAUSED:
                break;
              case init.firebase.default.storage.TaskState.RUNNING:
                break;
            }
          },
          (error) => {
