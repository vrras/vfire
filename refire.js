...
export default class Refire {
  ...
  constructor(credential) {
    const firebaseLib = init(credential);
    const firestore = firebaseLib.firebase.firestore();
    const storage = firebaseLib.firebase.storage();
    const auth = firebaseLib.firebase.auth();
    const timestamp = firebaseLib.firebase.firestore.FieldValue.serverTimestamp();

    const randomName = (type, length) => {
      let charset = '';
      let result = '';
    ...
    this.rawQuery = (query, coll, params, realtime = true) => {
      ...
      if (coll && splitRef.length > 1) {
        if (splitRef.length % 2 !== 0) {
          ...
          dbRef = this._firestore.doc(ref);
        } else {
          throw new Error('Invalid document reference.');
        }
      }

      const fireSQL = new this._firebaseLib.FireSQL(dbRef);
    ...
    this.getCollection = (params) => {
      return new Promise(async (resolve, reject) => {
        ...
        let ref = this._firestore.collection(splitColl[0]);
    ...
    this.addDocument = (params) => {
      return new Promise(async (resolve, reject) => {
        ...
        params.data.created = this._timestamp;
        params.data.updated = this._timestamp;
        ...
        await this._storage
          ...
          .then((dir) => {
            ...
            dir.prefixes.forEach((folderRef) => {
              ...
              deleteStorageDirectoryFirebase(folderRef.fullPath);
            });
          });
      });
    }
        ...
        uploadTask.on(
          ...
          (snapshot) => {
            switch (snapshot.state) {
              case this._firebaseLib.firebase.storage.TaskState.PAUSED:
                break;
              case this._firebaseLib.firebase.storage.TaskState.RUNNING:
                break;
            }
          },
          (error) => {
